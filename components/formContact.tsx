...
export default function FormContact({ form, setForm, onClose, allContact, setAllContact, getData, isNewContact, allNumber, setAllNumber }: any) {
  const index = useIndexStore((state: any) => state.index);
  ...
  const handleCreate = () => {
    ...
    const maxIdObject = allContact.reduce((max: any, current: any) => (current.id > max.id ? current : max), allContact[0]);
    const isConstantValid = regexPattern.test(form.first_name);
    const isNotUnique = allContact.some((item: any) => item.first_name === form.first_name);
  ...
  const handleUpdate = () => {
    const updatedData = allContact.map((item: any) => {
      if (item.id === index) {
    ...
    <div className="relative bg-greyOne shadow-md rounded-md flex justify-center">
      <div className="flex flex-col w-[90%] pt-[30px] pb-[10px] gap-1 content-center item-middle ">
        ...
        <div className="w-full flex gap-1 items-center align-middle ">
          <button onClick={handleAddNumber} className={` ${isNewContact ? 'w-[40%]' : 'w-[30%] text-[10px]'} px-1 py-1 text-white duration-300 ease-in-out bg-blue-500 rounded font-700`}>
            add number
          </button>
          <div className="w-full flex gap-1 items-center align-middle">
            {allNumber &&
              allNumber.map((data: any, index: number) => (
                <span
                  onClick={() => {
